# This code was slightly adapted from the original https://github.com/naver/roma/blob/master/roma/utils.py
# RoMa
...
from tools.transforms.geometric.quaternion import quat_composition, quat_product, quat_conjugation, quat_normalize, quat_abs, quat_product_scalar, quat_subtraction
...
def is_orthonormal_matrix(R, epsilon=1e-7):
    ...
    errors = torch.norm(R @ R.transpose(-1, -2) - torch.eye(D,
                        device=R.device, dtype=R.dtype), dim=[-2, -1])
...
def rotmat_cosine_angle(R):
    ...
    """
    assert R.shape[-2:] == (3,
                            3), "Expecting a ...x3x3 batch of rotation matrices"
...
def unitquat_slerp(q0, q1, steps, shortest_arc=True):
    ...
    rel_rotvec = roma.mappings.unitquat_to_rotvec(
        rel_q, shortest_arc=shortest_arc)
    # Relative rotations to apply
    rel_rotvecs = steps.reshape(steps.shape + (1,) * rel_rotvec.dim()) * \
        rel_rotvec.reshape((1,) * steps.dim() + rel_rotvec.shape)
    rots = roma.mappings.rotvec_to_unitquat(
        rel_rotvecs.reshape(-1, 3)).reshape(*rel_rotvecs.shape[:-1], 4)
    interpolated_q = quat_product(q0.reshape(
        (1,) * steps.dim() + q0.shape).repeat(steps.shape + (1,) * q0.dim()), rots)
...
def unitquat_slerp_fast(q0, q1, steps, shortest_arc=True):
    ...
    # Interpolation
    q = alpha * q0.reshape((1,) * steps.dim() + q0.shape) + \
        beta * q1.reshape((1,) * steps.dim() + q1.shape)
...
def rigid_vectors_registration(x, y, weights=None, compute_scaling=False):
    ...
    if compute_scaling:
        ...
        if weights is None:
            ...
            sigma2_x = torch.sum(
                weights * torch.sum(torch.square(x), dim=-1), dim=-1)
...
def rigid_points_registration(x, y, weights=None, compute_scaling=False):
    ...
    if weights is None:
        ...
        xmean = torch.sum(
            normalized_weights[..., None] * x, dim=-2, keepdim=True)
        ymean = torch.sum(
            normalized_weights[..., None] * y, dim=-2, keepdim=True)
    xhat = x - xmean
    yhat = y - ymean

    # Solve the vectors registration problem
    if compute_scaling:
        R, scale = rigid_vectors_registration(
            xhat, yhat, weights, compute_scaling=compute_scaling)
        t = (ymean - torch.einsum('...ik, ...jk -> ...ji',
             scale[..., None, None] * R, xmean)).squeeze(-2)
        return R, t, scale
    else:
        R = rigid_vectors_registration(
            xhat, yhat, weights, compute_scaling=compute_scaling)
...
@torch.jit.script
def hermite(x: torch.Tensor, p0: torch.Tensor, p1: torch.Tensor, v0: torch.Tensor, v1: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """Compute the Hermite quadratic spline interpolation between two points and there velocities.

    Parameters
    ----------
    x : torch.Tensor
        Step of the interpolation. Normally Range [0, 1].
    p0 : torch.Tensor
        Left point of the evaluation.
    p1 : torch.Tensor
        Right point of the evaluation.
    v0 : torch.Tensor
        Velocity / Tangent of the left point.
    v1 : torch.Tensor
        Velocity / Tangent of the right point.

    Returns
    -------
    Tuple[torch.Tensor, torch.Tensor]
        Position and velocity at the step x.
    """
    p1_sub_p0 = p1 - p0
    w1 = - 2 * x ** 3 + 3 * x ** 2
    w2 = x ** 3 - 2 * x ** 2 + x
...
@torch.jit.script
def hermite_position(x: torch.Tensor, p0: torch.Tensor, p1: torch.Tensor, v0: torch.Tensor, v1: torch.Tensor) -> torch.Tensor:
    """Compute the Hermite quadratic spline interpolation between two points and there velocities.

    Parameters
    ----------
    x : torch.Tensor
        Step of the interpolation. Normal Range [0, 1].
    p0 : torch.Tensor
        Left point of the evaluation.
    p1 : torch.Tensor
        Right point of the evaluation.
...
@torch.jit.script
def hermite_catmull_rom(
        x: torch.Tensor,
        p0: torch.Tensor,
        p1: torch.Tensor,
        p2: torch.Tensor,
        p3: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """Compute the Catmull-Rom cubic spline interpolation between four points.
    ...
    p1 : torch.Tensor
        Second point of the interpolation. (Left point of x)

    p2 : torch.Tensor
        Third point of the interpolation. (Right point of x)

    p3 : torch.Tensor
        Fourth point of the interpolation.
    ...
    v1 = ((p1 - p0) + (p2 - p1)) / 2
    v2 = ((p2 - p1) + (p3 - p2)) / 2
    return hermite(x, p1, p2, v1, v2)


@torch.jit.script
def hermite_catmull_rom_position(
        x: torch.Tensor,
        p0: torch.Tensor,
        p1: torch.Tensor,
        p2: torch.Tensor,
    ...
    v1 = ((p1 - p0) + (p2 - p1)) / 2
    v2 = ((p2 - p1) + (p3 - p2)) / 2
    return hermite_position(x, p1, p2, v1, v2)


@torch.jit.script
def quat_hermite(x: torch.Tensor, r0: torch.Tensor, r1: torch.Tensor, v0: torch.Tensor, v1: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """Compute the Hermite quadratic spline interpolation between two quaterion rotations and there angular velocities.
    ...
    x : torch.Tensor
        Step of the interpolation. Normally Range [0, 1].
    r0 : torch.Tensor
        Left Quaternion of the evaluation.
    r1 : torch.Tensor
        Right Quaternion of the evaluation.
    v0 : torch.Tensor
        Velocity / Tangent of the left point.
    v1 : torch.Tensor
        Velocity / Tangent of the right point.
    ...
    -------
    Tuple[torch.Tensor, torch.Tensor]
        Rotation and velocity at the step x.
    """
    r1_sub_r0 = unitquat_to_rotvec(quat_subtraction(r1, r0))
    w1 = - 2 * x ** 3 + 3 * x ** 2
    w2 = x ** 3 - 2 * x ** 2 + x
    w3 = x ** 3 - x ** 2

    rot = quat_product(rotvec_to_unitquat(
        w1 * r1_sub_r0 + w2 * v0 + w3 * v1), r0)
...
@torch.jit.script
def quat_hermite_position(x: torch.Tensor, r0: torch.Tensor, r1: torch.Tensor, v0: torch.Tensor, v1: torch.Tensor) -> torch.Tensor:
    """Compute the Hermite quadratic spline interpolation between two quaterion rotations and there angular velocities.
    ...
    x : torch.Tensor
        Step of the interpolation. Normally Range [0, 1].
    r0 : torch.Tensor
        Left Quaternion of the evaluation. Shape (..., 4).
    r1 : torch.Tensor
        Right Quaternion of the evaluation. Shape (..., 4).
    v0 : torch.Tensor
        Velocity / Tangent of the left point in form of a rotation vector. Shape (..., 3).
    v1 : torch.Tensor
        Velocity / Tangent of the right point in form of a rotation vector. Shape (..., 3).
...
@torch.jit.script
def quat_hermite_catmull_rom(
        x: torch.Tensor,
        r0: torch.Tensor,
        r1: torch.Tensor,
        r2: torch.Tensor,
        r3: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """Compute the Catmull-Rom cubic spline interpolation between four points.
    ...
    Tuple[torch.Tensor, torch.Tensor]
        Position at the step x.
        Velocity at the step x.
    """

    r1_sub_r0 = unitquat_to_rotvec(quat_subtraction(r1, r0))
    r2_sub_r1 = unitquat_to_rotvec(quat_subtraction(r2, r1))
    r3_sub_r2 = unitquat_to_rotvec(quat_subtraction(r3, r2))

    v1 = (r1_sub_r0 + r2_sub_r1) / 2
    v2 = (r2_sub_r1 + r3_sub_r2) / 2
    return quat_hermite(x, r1, r2, v1, v2)


@torch.jit.script
def quat_hermite_catmull_rom_position(
        x: torch.Tensor,
        r0: torch.Tensor,
        r1: torch.Tensor,
        r2: torch.Tensor,
        r3: torch.Tensor) -> torch.Tensor:
    """Compute the Catmull-Rom cubic spline interpolation between four points.
    ...
    v1 = (r1_sub_r0 + r2_sub_r1) / 2
    v2 = (r2_sub_r1 + r3_sub_r2) / 2
    return quat_hermite_position(x, r1, r2, v1, v2)
